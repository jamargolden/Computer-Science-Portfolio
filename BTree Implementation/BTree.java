 ...
 * 
 * TODO: Complete the following three tasks in the order shown:
 * 1. Implement the Node.splitChild() method.
 * 2. Handle the root full case in BTree.insert().
 * 3. Ignore attempt to insert a duplicate key.
 */

class BTree {
  
  /**
   * A structure to pinpoint the location of a key in the tree.
   * Used as the return type of the search() method.
   */
  class Location {
    Node p;
    int i;
  ...
  class Node {
    ...
    List<Node> children = new ArrayList<>(b + 1);  // null pointers are not stored
    ...
    void splitChild(int i) {
      ...
      int j = (b - 1)/ 2;
      keys.add(i, children.get(i).keys.get(j));

      for(int l = j+1; l < b; l++){
        right.keys.add(children.get(i).keys.get(l));
        children.get(i).keys.remove(l);
        if (children.get(i).children.size() >= l){
          right.children.add(children.get(i).children.get(l));
          children.get(i).children.remove(l);
        }
      }

      children.add(i+1, right);
      children.get(i).keys.remove(j);
    ...
    void insert(int key) {
       ...
       * tree.  Make the appropriate changes to ensure that an attempt
       * to insert a duplicate keys does nothing.
       */
      ...
      if (isLeaf()) {
        ...
        // There's guaranteed to be room in this node, so go ahead and
        // insert.
